      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            if (row.link == "") {
              link_btn +=
                '<a class="me-2 btn btn-sm py-0 btn-secondary disabled" href="#">No</a>';
            } else {
              link_btn +=
                '<a class="me-2 btn btn-sm py-0 btn-secondary" href="' +
                row.link +
                '" target="_blank">Sí</a>';
            }
            return link_btn;
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            let extra_btns = "";
            extra_btns +=
              '<a class="me-2 btn btn-sm py-0 edit_data btn-primary" href="javascript:void(0)" data-id="' +
              row.idCliente +
              '">Editar</a>';
            extra_btns +=
              '<a class="btn btn-sm py-0 delete_data btn-danger" href="javascript:void(0)" data-id="' +
              row.idCliente +
              '">Eliminar</a>';
            return extra_btns;
          },
            ...
            success: function (resp) {
              if (!!resp.status) {
                Object.keys(resp.data).map((k) => {
                  if (
                    $("#edit_modal").find('input[name="' + k + '"]').length > 0
                  )
                    $("#edit_modal")
                      .find('input[name="' + k + '"]')
                      .val(resp.data[k]);
        ...
        $(".delete_data").click(function () {
          $.ajax({
            ...
            success: function (resp) {
              if (!!resp.status) {
                $("#delete_modal")
                  .find('input[name="idCliente"]')
                  .val(resp.data["idCliente"]);
                $("#delete_modal")
                  .find('span[name="codigo"]')
