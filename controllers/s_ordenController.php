...
switch ($op) {
    ...
    case 1: {
            $db = new Database();
            $con = $db->conectar();
            $create = $con->prepare("INSERT INTO $tblName (`numero`,`fec_emision`,`numero_siaf`,`link`,`idServicio`) VALUES (?,?,?,?,?)");
            $create->execute([$numero, $fec_emision, $numero_siaf, $link, $idServicio]);
            if ($create) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 2: {
            try {
                ...
                if (!empty($search)) {
                    ...
                    $search_where .= " OR numero_siaf LIKE '%{$search['value']}%' ";
    ...
    case 3: {
            $db = new Database();
            $con = $db->conectar();
            $update = $con->prepare("UPDATE $tblName SET `numero` = ?, `fec_emision` = ?, `numero_siaf` = ?, `link` = ?, `idServicio` = ? WHERE idOrden = ?");
            $update->execute([$numero, $fec_emision, $numero_siaf, $link, $idServicio, $idOrden]);
            if ($update) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 4: {
            $db = new Database();
            $con = $db->conectar();
            $delete = $con->prepare("DELETE FROM $tblName WHERE idOrden = ?");
            $delete->execute([$idOrden]);
            if ($delete) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 5: {
            try {
                $db = new Database();
                $con = $db->conectar();
                $query = $con->prepare("SELECT * FROM $tblName WHERE idOrden = ?");
                $query->execute([$idOrden]);
                if ($query) {
                    $resp['status'] = 'success';
