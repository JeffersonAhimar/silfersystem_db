...
switch ($op) {
    ...
    case 1: {
            $db = new Database();
            $con = $db->conectar();
            $create = $con->prepare("INSERT INTO $tblName (`fecha`,`link`,`idEntregable`) VALUES (?,?,?)");
            $create->execute([$fecha, $link, $idEntregable]);
            if ($create) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 2: {
            try {
                ...
                if (!empty($search)) {
                    ...
                    $search_where .= " OR idEntregable LIKE '%{$search['value']}%' ";
                }
                // NOMBRES DE COLUMNAS
                $columns_arr = array(
                    "idConformidad",
    ...
    case 3: {
            $db = new Database();
            $con = $db->conectar();
            $update = $con->prepare("UPDATE $tblName SET `fecha` = ?, `link` = ?, `idEntregable` = ? WHERE idConformidad = ?");
            $update->execute([$fecha, $link, $idEntregable, $idConformidad]);
            if ($update) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 4: {
            $db = new Database();
            $con = $db->conectar();
            $delete = $con->prepare("DELETE FROM $tblName WHERE idConformidad = ?");
            $delete->execute([$idConformidad]);
            if ($delete) {
                $resp['status'] = 'success';
            } else {
                $resp['status'] = 'failed';
    ...
    case 5: {
            try {
                $db = new Database();
                $con = $db->conectar();
                $query = $con->prepare("SELECT * FROM $tblName WHERE idConformidad = ?");
                $query->execute([$idConformidad]);
                if ($query) {
                    $resp['status'] = 'success';
